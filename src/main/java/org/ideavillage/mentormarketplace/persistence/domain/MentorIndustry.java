...
@XmlRootElement
@NamedQueries({
    @NamedQuery(name = "MentorIndustry.findAll", query = "SELECT m FROM MentorIndustry m"),
    @NamedQuery(name = "MentorIndustry.findById", query = "SELECT m FROM MentorIndustry m WHERE m.id = :id")})
public class MentorIndustry implements Serializable {
    ...
    public MentorIndustry(Integer id) {
        this.id = id;
    }
